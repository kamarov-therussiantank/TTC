// UI Tank Icon and UI Tank Icon Loader
...
UITankIcon.classMethod('drawTankIcon', function (
    ...
    if (
        canvas.width !== this.compositedBuffer.width ||
        canvas.height !== this.compositedBuffer.height
    ) {
    ...
    if (
        canvas.width !== this.tintedBuffer.width ||
        canvas.height !== this.tintedBuffer.height
    ) {
    ...
    [turretAccessory, frontAccessory, barrelAccessory].forEach(image => {
        if (image instanceof HTMLImageElement) {
            compositedContext.drawImage(image, 0, 0, canvas.width, canvas.height);
        }
    });
    if (!UIConstants.DISABLE_TANK_BADGES && badge instanceof HTMLImageElement) {
...
UITankIconLoader.method('_queueImage', function (
    ...
    if (cachedImage === undefined) {
        ...
        let src =
            d_url(
                'https://raw.githubusercontent.com/kamarov-therussiantank/TTC/refs/heads/main/src/assets/images/' +
                imageName +
                '.png'
            );
        let srcset =
