//UIGameIconImage adjustments & additional settings to choose game icon count
...
if (!isNaN(savedIconCount)) {
    ...
    UIConstants.GAME_ICON_POOL_SIZE = 3;
    UIConstants.GAME_ICON_COUNT = 3;
}
...
UIConstants.generateTankIconPlacements = function(count) {
    ...
    if (count !== 6) {
        console.warn("This layout is designed for 6 tanks. Adjust accordingly for other counts.");
    }
...
UIConstants.generateTankIconPlacements(6);
    ...
    for (var i = 0; i < TankTrouble._gameIconInstances.length; ++i) {
        try {
            ...
            if (inst.playerStates) {
                ...
                inst.iconPlacements = createCircle(tankCount, radiusX, radiusY);
                inst._updateUI();
